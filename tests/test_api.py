...
class APITestCase(unittest.TestCase):  # 测试类从unittest.TestCase继承
    def setUp(self):  # setUp方法在每调用一个测试方法前执行
    ...
    def tearDown(self):  # tearDown方法在每调用一个测试方法后执行
        db.session.remove()
        db.drop_all()
